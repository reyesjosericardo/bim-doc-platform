...
const BLOCKS = ['Proyecto', 'Objetivos', 'Hitos', 'LOIN', 'Entrega'];

const REQUIRED_BY_BLOCK: Record<number, string[]> = {
  0: ['PIR-1.1', 'PIR-1.2', 'PIR-1.4', 'PIR-1.5'],
  1: ['PIR-2.1', 'PIR-2.5'],
  2: ['PIR-3.1', 'PIR-3.3'],
  3: ['PIR-4.1'],
  4: ['PIR-5.1', 'PIR-5.2'],
...
function inferAnswerType(questionId: string): string {
  const textarea = ['PIR-1.5', 'PIR-2.4', 'PIR-3.2', 'PIR-5.4', 'PIR-5.5'];
  const boolean = ['PIR-2.5', 'PIR-3.3', 'PIR-4.1', 'PIR-5.3'];
  const multi = ['PIR-2.1', 'PIR-3.1', 'PIR-3.4', 'PIR-4.3', 'PIR-4.4', 'PIR-5.1', 'PIR-5.2'];
  const select = ['PIR-1.2', 'PIR-2.6', 'PIR-4.2'];
...
export function PIRForm({ documentId, projectId, initialAnswers = {}, initialStatus = 'borrador' }: PIRFormProps) {
  ...
  async function saveBlock() {
    ...
    try {
      if (!docIdRef.current) {
        const res = await fetch('/api/documents/pir', {
          method: 'POST',
        ...
        const res = await fetch(`/api/documents/pir/${docIdRef.current}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function handleStatusChange(newStatus: DocumentStatus) {
    ...
    try {
      const res = await fetch(`/api/documents/pir/${docIdRef.current}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div className="max-w-3xl mx-auto px-4 py-3.5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="font-display text-[15px] text-[#EEE9DB] leading-tight truncate">PIR — Requisitos de Información del Proyecto</h1>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {activeBlock === BLOCKS.length - 1 && docIdRef.current && (
          <div className="card mb-6">
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
              ...
              Exportar documento PIR
            </h3>
            <GenerateDocumentButton documentId={docIdRef.current} status={status} answeredCount={totalAnswered} docType="PIR" />
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          {activeBlock < BLOCKS.length - 1 ? (
            ...
            <button onClick={handleFinish} disabled={saving} className="btn-primary">{saving ? 'Guardando...' : 'Finalizar PIR'}</button>
          )}
        </div>
      </div>
    </div>
