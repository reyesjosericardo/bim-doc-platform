...
function calcProgress(answers: { question_id: string; answer_value: string }[]) {
  ...
  const answered = answers.filter(({ question_id, answer_value }) => {
    ...
    if (conditionalIds.has(question_id)) return answerMap[CONDITIONAL_TRIGGERS[question_id]] === 'Sí';
    return true;
  }).length;
...
export async function GET(_req: Request, { params }: { params: { projectId: string } }) {
  ...
  const documents = await prisma.bimDocument.findMany({
    where: { project_id: params.projectId, document_type: 'PIR' },
    include: {
  ...
  const withProgress = documents.map((doc) => {
    ...
    return { ...doc, answered_count: answered, total_questions: total, progress_pct: pct };
  });
