...
export async function GET(
  ...
  const res = await fetch(
    `${backendUrl}/api/documents/air/${params.id}/download/${params.format}`,
  ...
  const disposition = res.headers.get('content-disposition') ?? `attachment; filename="AIR.${params.format}"`;
