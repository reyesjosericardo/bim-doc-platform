...
export async function GET(
  ...
  const res = await fetch(
    `${backendUrl}/api/documents/oir/${params.id}/download/${params.format}`,
  ...
  const disposition = res.headers.get('content-disposition') ?? `attachment; filename="OIR.${params.format}"`;
