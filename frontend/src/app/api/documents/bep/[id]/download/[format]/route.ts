...
export async function GET(
  ...
  const res = await fetch(
    `${backendUrl}/api/documents/bep/${params.id}/download/${params.format}`,
  ...
  const disposition = res.headers.get('content-disposition') ?? `attachment; filename="BEP.${params.format}"`;
