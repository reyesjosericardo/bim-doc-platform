...
export async function GET(
  ...
  const res = await fetch(
    `${backendUrl}/api/documents/pir/${params.id}/download/${params.format}`,
  ...
  const disposition = res.headers.get('content-disposition') ?? `attachment; filename="PIR.${params.format}"`;
