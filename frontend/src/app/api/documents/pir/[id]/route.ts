...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const document = await prisma.bimDocument.findUnique({
    where: { id: params.id, document_type: 'PIR' },
    include: {
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const document = await prisma.bimDocument.findUnique({ where: { id: params.id, document_type: 'PIR' } });
  if (!document) return NextResponse.json({ error: 'Not found' }, { status: 404 });
  ...
  await Promise.all(
    answers.map((a: any) =>
      prisma.questionnaireAnswer.upsert({
        ...
        create: { document_id: params.id, question_id: a.question_id, answer_value: a.answer_value, answer_type: a.answer_type },
      })
    )
  );

  await prisma.bimDocument.update({ where: { id: params.id }, data: { updated_at: new Date() } });
